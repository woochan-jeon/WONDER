...
import { z } from "zod";
import { appendLedgerEntry, updateLedgerEntry, LEDGER_PROJECTS } from "@/lib/google-sheets";
...
export async function updateLedgerEntryAction(
  ...
  const { projectId, row, ...input } = parsed.data;
  const project = LEDGER_PROJECTS.find((p) => p.id === projectId);
  if (!project) return { error: "프로젝트를 찾을 수 없습니다" };

  try {
    await updateLedgerEntry(project, row, { ...input, note: input.note ?? "", receipt: input.receipt ?? "" });
  } catch {
    return { error: "시트를 수정하는 중 오류가 발생했습니다. 연결을 확인해 주세요." };
  }
